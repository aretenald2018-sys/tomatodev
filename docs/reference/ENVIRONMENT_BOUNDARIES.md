...
FCM is fail-closed on every TomatoDev platform. The public initialization and token APIs remain callable but return `tomatodev-fcm-disabled`; they never request notification permission, invoke Capacitor push registration, load Firebase Messaging, or write/delete `_fcm_tokens`. The retained Android source does not apply the Google Services plugin. Production Firebase callable functions are blocked before a proxy is created. This repository publishes only its own development APK, built under the dedicated application ID `com.lifestreak.dev` and downloaded from `public/downloads/tomatodev.apk`. The operating app's `tomato-mobile-debug.apk` is never referenced, staged, or downloaded here. That APK is built locally with the stable Android debug key and committed, because a CI runner mints a new debug keystore on every run and a differently signed build cannot install over the previous one. Do not deploy Firebase or Functions from this repository.

## Watch running records

Wear Data Layer transfers are scoped to one application ID and signing key. A watch app therefore reaches only the phone app that shares both. The operating watch app (`com.lifestreak.app`) delivers a finished run to the operating phone app alone, so TomatoDev's `TomatoWearWorkoutListenerService` is never invoked for it. A watch sends its run to TomatoDev only when the TomatoDev watch app, built under the same dedicated application ID `com.lifestreak.dev` and the same local debug key, is installed on that watch. Both watch apps can stay installed side by side; each run goes to the app it was started in.

`npm.cmd run build:mobile-apk` builds and publishes both surfaces (`public/downloads/tomatodev.apk`, `public/downloads/tomatodev-wear.apk`) and raises each module's `versionCode`; `npm.cmd run build:wear-apk` builds the watch surface alone. `npm.cmd run install:wear-watch` sideloads the watch APK over adb and verifies `com.lifestreak.dev`, falling back to the published artifact when no local Gradle output exists. The watch APK is never installed from a store: the manual refresh path's remote install prompt cannot resolve a development application ID, so TomatoDev reports that adb installation is required instead of claiming a store screen opened.

Once a run is saved by either phone app, both environments display it, because Firebase project `exercise-management` is still shared: the day document under `users/{owner}/workouts/{dateKey}`, its `runRouteRef`, and the `users/{owner}/running_routes/{routeId}` chunks are one copy read live by both apps. Only the Wear ingest hop is per-application; the record itself is not.
