...
- 변경:
  ...
  4. `sw.js`: `STATIC_ASSETS` 대상 변경에 맞춰 `CACHE_VERSION`을 `tomatofarm-v20260623z2-diet-report-off`로 bump했다.
...
- 로컬 검증:
  ...
  9. PASS: `GET http://localhost:5500/feature-diet-premium-report.js` — HTTP 200, disabled guard 포함
  10. PASS: 브라우저 DOM 확인 — `#diet-premium-report-modal` 0개, `식단 프리미엄 리포트` 텍스트 미노출
  11. 참고: 브라우저 세션은 로그인 완료 상태가 아니라 `window.__tomatoAppReady`는 false였지만, 자동 노출 함수가 모듈 최상단 guard에서 종료되므로 기존 target 사용자/inbox 상태와 무관하게 모달 생성 경로가 닫힌다.
