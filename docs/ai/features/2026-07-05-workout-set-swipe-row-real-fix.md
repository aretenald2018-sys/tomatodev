...
6. capture 전환 후에도 `saveDay()`의 dateKey 직렬화 큐 때문에 `_cache` 갱신이 뒤로 밀리면, optimistic render가 stale `getCache()`를 읽어 삭제 전 row를 다시 그릴 수 있다.
...
1. `render-calendar.js`
   ...
   - optimistic render 전에 `saveDay(merge)`와 같은 방식으로 해당 날짜 local cache를 즉시 merge한다.
...
3. `tests/workout-calendar-bottom-sheet.test.js`
   ...
   - optimistic render가 stale cache를 읽지 않도록 render 전 local cache merge를 고정한다.
4. `sw.js`
