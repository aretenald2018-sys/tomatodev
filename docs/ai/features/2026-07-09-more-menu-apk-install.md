...
## 사용자 피드백: 모바일 APK 구버전 자산 수정 (2026-07-09)

### 증상

다운로드되는 모바일 APK를 설치하면 최신 Tomato Farm 화면이 아니라 구버전처럼 보인다. 특히 라이프존 식사 사진 말풍선의 밑동이 예전 마름모 형태처럼 보이고, 식사 사진 말풍선의 하트 버튼이 눌리지 않는다.

### 진단 결과

1. 현재 root `sw.js`는 `tomatofarm-v20260709z10-mobile-apk-download`다.
2. `public/downloads/tomato-mobile-debug.apk` 내부 `assets/public/sw.js`와 `assets/public/build-info.json`은 `tomatofarm-v20260709z4-more-menu-apk-deploy`다.
3. `www/sw.js`와 `android/app/src/main/assets/public/sw.js`도 같은 `z4`라서, APK 빌드 입력 자산 자체가 최신 root와 동기화되지 않았다.
4. APK 내부 `assets/public/style.css`는 최신 `.lz-photo-like-btn`/말풍선 꼬리 polish 계약을 포함하지 않는다.

### 결정

1. root 소스는 이미 최신 life-zone 사진 말풍선/하트 수정이 들어 있으므로 UI 코드를 새로 바꾸지 않는다.
2. 이번 수정은 root 정적 자산 내용 변경이 아니라 APK 포장 누락 수정이므로 `scripts/bump-cache.js`는 실행하지 않는다.
3. `node scripts/generate-build-info.mjs`와 `node scripts/copy-www.js`로 `build-info.json`과 `www/`를 최신 root 기준으로 다시 생성한다.
4. `npx.cmd cap sync android`로 `android/app/src/main/assets/public/`을 최신 `www/`와 동기화한다.
5. Android debug APK를 다시 빌드하고 `public/downloads/tomato-mobile-debug.apk`를 새 산출물로 교체한다.
6. 회귀 테스트는 APK zip 내부를 직접 읽어 `assets/public/sw.js`, `assets/public/build-info.json`, `assets/public/home/life-zone.js`, `assets/public/style.css`가 최신 life-zone photo bubble 계약을 담는지 검사한다.

## 실행 Slice 4: 모바일 APK 최신 자산 재빌드

### 범위

1. Generated/package assets
   - `node scripts/generate-build-info.mjs`로 root `build-info.json`을 현재 `sw.js` marker와 동기화한다.
   - `node scripts/copy-www.js`로 `www/`를 최신 root source에서 재생성한다.
   - `npx.cmd cap sync android`로 Android WebView asset을 동기화한다.
   - Android `:app:assembleDebug`를 실행해 모바일 debug APK를 재빌드한다.
   - 새 `android/app/build/outputs/apk/debug/app-debug.apk`를 `public/downloads/tomato-mobile-debug.apk`로 복사한다.

2. Tests
   - `tests/wear-app-refresh-update.test.js`에 APK zip entry reader를 추가한다.
   - 공개 모바일 APK가 root `sw.js`와 같은 `CACHE_VERSION`을 담고, life-zone photo-like JS/CSS 계약을 포함하지 않으면 실패하게 한다.

3. Documentation
   - 이 계획과 `docs/ai/NEXT_ACTION.md`에 구버전 APK 원인, 실행 결과, 검증을 기록한다.

### 하지 않는 것

- `home/life-zone.js`/`style.css` UI 동작을 새로 리디자인하지 않는다.
- `www/`를 손으로 직접 수정하지 않는다. 생성 스크립트와 Capacitor sync 결과만 사용한다.
- 워치 APK 다운로드 경로를 되살리지 않는다.

### 검증 계획

1. RED: `node --test tests/wear-app-refresh-update.test.js`가 기존 APK 내부 `z4` 때문에 실패한다.
2. PASS 목표: `node scripts/generate-build-info.mjs && node scripts/copy-www.js`.
3. PASS 목표: `npx.cmd cap sync android`.
4. PASS 목표: Android `:app:assembleDebug`.
5. PASS 목표: `node --test tests/wear-app-refresh-update.test.js`.
6. PASS 목표: `npm.cmd run verify:assets`.
7. PASS 목표: `node --test tests/app-shell-action-bridge.test.js tests/wear-app-refresh-update.test.js tests/pwa-update-auto-reload.test.js`.
8. Production deploy: 변경 커밋을 `origin/main`에 push한 뒤 `npm.cmd run verify:deploy -- https://aretenald2018-sys.github.io/tomatofarm/ <commit>`를 통과시킨다.
9. Production-flow QA: 배포 URL에서 `더보기 -> APK 설치하기`가 새 `tomato-mobile-debug.apk`를 다운로드하고, 다운로드된 APK 내부 `assets/public/sw.js`가 최신 cache marker인지 확인한다.

### 실행 결과

1. `node scripts/generate-build-info.mjs`와 `node scripts/copy-www.js`로 최신 root asset을 `www/`에 재생성했다.
2. 기존 checkout의 설치된 Capacitor CLI를 `NODE_PATH`로 연결해 `cap sync android`를 실행했고, Android WebView assets를 최신 `tomatofarm-v20260709z10-mobile-apk-download`로 동기화했다.
3. 새 worktree에 빌드용 Android wrapper/settings/resource 파일만 임시 보강해 `:app:assembleDebug`를 실행했다. 이 임시 파일들은 커밋에 포함하지 않았다.
4. 새 `android/app/build/outputs/apk/debug/app-debug.apk`를 `public/downloads/tomato-mobile-debug.apk`로 복사했다. 새 APK 크기는 `39,511,153 bytes`다.
5. `tests/wear-app-refresh-update.test.js`에 APK zip 내부 `assets/public/sw.js`, `build-info.json`, `home/life-zone.js`, `style.css`를 직접 검사하는 회귀 테스트를 추가했다.

### 검증 결과

1. PASS RED: `node --test tests/wear-app-refresh-update.test.js`가 기존 APK 내부 `tomatofarm-v20260709z4-more-menu-apk-deploy`와 root `tomatofarm-v20260709z10-mobile-apk-download` 불일치로 실패했다.
