...
## 후속 Slice 17 — Remove routine action and reserve calendar scroll gutter

### 진단

첨부 화면에서 하단 sheet header의 `루틴` 버튼이 우측 스크롤 indicator와 가까워 겹쳐 보이고, 월간 운동 캘린더의 마지막 날짜 열도 우측 스크롤 indicator 아래에 걸린다.

필요한 계약은 다음 두 가지다.

1. 하단 sheet header에서는 `오늘`만 남기고 `루틴` 버튼을 제거한다.
2. 운동 캘린더 홈 월간 grid는 우측 scroll indicator와 겹치지 않도록 안전 gutter를 확보한다.

### 범위

- `render-calendar.js`
  - `_renderWorkoutHomeDayBar()`에서 `루틴` action 버튼만 제거한다.
  - 기존 `_openWorkoutHomeRoutine()` 함수와 다른 루틴 진입 경로는 유지한다.
- `style.css`
  - `.workout-calendar-root`에 right padding을 CSS 변수 기반으로 적용하고 `scrollbar-gutter: stable`을 둔다.
  - `.cal-workout-surface-home`에서 기본/모바일 우측 gutter 값을 지정한다.
- `tests/workout-calendar-bottom-sheet.test.js`
  - sheet header에 `오늘`만 남고 `루틴` 버튼이 없는 source contract를 추가한다.
  - 월간 캘린더 우측 gutter source contract를 추가한다.
- `sw.js`와 cache marker 테스트들
  - `render-calendar.js`, `style.css`가 `STATIC_ASSETS`에 포함되므로 `CACHE_VERSION`을 bump한다.
...
- `git diff --check`
- 배포 시 `npm.cmd run verify:deploy -- https://aretenald2018-sys.github.io/dashboard3/ <commit>`
- 인증 계정 실제 flow: `운동 탭 -> 월간 캘린더 우측 날짜 열 확인`, `하단 sheet header에 루틴 버튼 미노출 확인`
