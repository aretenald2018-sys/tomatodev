...
- 현재 운영 cache marker: `tomatofarm-v20260704z3-running-lock-gps-recovery`
...
7. PASS: `npm.cmd run verify:deploy -- https://aretenald2018-sys.github.io/tomatofarm/ e43f24e2ef9b57f847a9ad80d8f2f966d1bb7a18`
   - `[deploy-verify] ok e43f24e2ef9b tomatofarm-v20260704z3-running-lock-gps-recovery static=242`
8. PASS: `npm.cmd run verify:deployed-markers -- https://aretenald2018-sys.github.io/tomatofarm/ sw.js::tomatofarm-v20260704z3-running-lock-gps-recovery render-calendar.js::WORKOUT_SET_TYPE_OPTIONS render-calendar.js::toggle-set-type render-calendar.js::set-set-type render-calendar.js::data-wt-set-type-option style.css::wt-max-set-type-menu workout/exercises.js::_firstTestModePrescriptionSet workout/expert/max-benchmark-picker.js::_firstPickerSetFromPrescription tests/workout-calendar-bottom-sheet.test.js::wt-max-set-type-btn tests/calc.max.test.js::maxPrescription.sets.length`
9. PASS: 운영 URL browser 확인
   - URL: `https://aretenald2018-sys.github.io/tomatofarm/`
   - title: `토마토 키우기`
   - 로그인 화면과 앱 shell 표시
   - console error 0건
10. not verified yet: 인증 세션이 없어 운영 URL에서 실제 `운동 탭 -> 종목 카드 -> 세트 유형 메뉴/우측 편집` 클릭 flow는 자동으로 수행하지 못했다.
...
- 세트 유형 메뉴의 click mutation 경로는 source-regex 테스트 비중이 높다. 차단 이슈는 아니지만, 다음 개선 시 DOM fixture 기반 action test로 보강하는 것이 좋다.
