...
## 2026-06-30 stale UI prune 추가 배포
...
- 브랜치: `main`
- URL: `https://aretenald2018-sys.github.io/tomatofarm/`
- 배포 기준 커밋: `c5fd880d243b`
- 포함된 최근 변경:
  - 화면 미구현 stale UI/API/route/style 제거
  - 홈 농장 stale surface 제거
  - 운동 record/detail stale route 및 standalone detail surface 제거
  - 전체통계 `setPeriod` cached HTML 호환 제거
  - 성장판 미리보기 미사용 `recommendationHtml` 파라미터 제거

### 확인한 사항

- `tomatofarm/main`은 현재 HEAD의 조상이라 운영계 전용 미반영 커밋을 덮어쓰지 않는 fast-forward push였다.
- `git push tomatofarm HEAD:main`으로 `0488ed7..c5fd880` 범위를 운영계에 반영했다.

### 검증

- PASS: `npm.cmd run verify:deploy -- https://aretenald2018-sys.github.io/tomatofarm/ c5fd880`
  - 결과: `c5fd880d243b`, `tomatofarm-v20260630z12-stale-ui-prune`, `static=233`
- PASS: `npm.cmd run verify:deployed-markers -- https://aretenald2018-sys.github.io/tomatofarm/ ...`
  - `sw.js::tomatofarm-v20260630z12-stale-ui-prune`
  - `app.js::sheet:tab-open`
  - `workout/navigation-stack.js::CALENDAR: 'CalendarScreen'`
  - `index.html::wt-running-session-root`
- not verified yet: 인증 계정 실제 UI에서 홈/운동/통계 화면 클릭 흐름은 직접 조작하지 않았다.
