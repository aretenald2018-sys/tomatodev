...
test('calcExerciseCalorieCredit · 러닝은 임의 250kcal 대신 신뢰 가능한 기록값만 반영', () => {
  const plan = { advancedMode: true, exerciseCalorieCredit: true, exerciseKcalRunning: 250 };
  assert.strictEqual(calcExerciseCalorieCredit(plan, { running: true }), 0);
  assert.strictEqual(calcExerciseCalorieCredit(plan, {
    running: true,
    runRouteSummary: {
      calories: 318,
