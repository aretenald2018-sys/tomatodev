...
test('시즌 마법사는 종목·목표를 통합한 네 단계와 트랜잭션 저장 진입을 제공한다', () => {
  const source = read('workout/season-manager.js');
  const styles = read('styles/features/seasons.css');
  assert.match(source, /기간.*종목·목표.*러닝.*미리보기/s);
  assert.match(source, /GROUP_LABELS.*가슴.*등.*어깨.*하체.*팔.*복부/s);
  assert.match(source, /SEASON_NORMAL_INCREMENTS_KG/);
  assert.match(source, /data-season-wendler-map/);
  assert.match(source, /data-season-action="select-group" data-season-group/);
  assert.match(source, /data-season-program="program"/);
  assert.match(source, /data-season-normal="baselineKg".*data-season-normal="incrementKg"/s);
  assert.doesNotMatch(source, /type="radio"[^>]*data-season-normal="incrementKg"/);
  ...
  assert.match(source, /등록 운동의 ID와 이름을 그대로 사용합니다/);
