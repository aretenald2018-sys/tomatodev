...
test('시즌 마법사는 선택 확인을 포함한 다섯 단계와 생성·수정 저장 진입을 제공한다', () => {
  ...
  assert.match(source, /data-season-action="running-pace-mode"/);
  assert.match(source, /seasonPresetEndDate\(_state\.season\.startDate, weeks\)/);
  assert.match(source, /data-season-running-pace-field="minutes"/);
  assert.match(source, /data-season-running-pace-field="seconds"/);
  assert.match(source, /data-season-exercise-window="startDate"/);
  assert.match(source, /RUNNING_ADAPTIVE_RATE_OPTIONS/);
  ...
  assert.match(source, /buildSeasonStairOverrideDraft\(\s*_state\.overrides\[configuredExerciseId\]/s);
...
test('현재 시즌 성장 보드는 시즌 문서와 활성 보드를 한 트랜잭션으로 함께 저장한다', () => {
  const api = read('data/data-api.js');
  assert.match(api, /const seasonKey = currentSeason \? tomatoDevSeasonBoardKey\(currentSeason\.id\) : null/);
  assert.match(api, /TOMATODEV_ACTIVE_SEASON_BOARD_KEY/);
  ...
  assert.doesNotMatch(api, /const activeRef = _doc\('settings', 'test_board_v2'\)/);
});
