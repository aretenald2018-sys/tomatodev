...
test('normalizeFromCsv · 가공식품 CSV → per_100g base + 1회 제공량 추정 포함', () => {
  const csv = {
    id: 'csv_123',
    name: '콜라',
    manufacturer: '코카콜라',
  ...
  assert.equal(est.label, '1회 제공량 250g');
  // default: 1회 제공량 추정이 있으면 serving_est
  assert.equal(c.defaultServingId, 'serving_est');
});

test('normalizeFromCsv · 견과류는 100g 대신 30g 1회 제공량이 기본', () => {
  const c = normalizeFromCsv({
    id: 'csv_nuts',
    name: '견과파워',
    manufacturer: '동우농산',
    energy: 588, protein: 13, carbs: 42, fat: 41, sodium: 0,
  });
  const serving = c.servings.find(s => s.id === c.defaultServingId);
  assert.deepEqual(serving, { id: 'serving_est', label: '1회 제공량 30g', grams: 30 });
});
