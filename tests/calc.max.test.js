...
test('buildMaxCycleSnapshot · 사이클 시작 전 기준 기록은 latest가 아니라 baselineLatest로 분리한다', () => {
  const cycle = {
    id: 'cycle_baseline_scope',
    status: 'active',
    framework: 'dual_track_progression_v2',
    startDate: '2026-05-04',
    weeks: 6,
    benchmarks: [{
      ...
      primaryMajor: 'chest',
      tracks: {
        M: { startKg: 100, targetKg: 102.5, incrementKg: 2.5, startReps: 12, targetReps: 12, enabled: true },
        H: { startKg: 110, targetKg: 112.5, incrementKg: 2.5, startReps: 8, targetReps: 6, enabled: true },
      },
    }],
  };
  const exList = [{ id: 'ex_bench', movementId: 'barbell_bench', name: '바벨 벤치프레스' }];
  const cache = {
    '2026-04-30': {
      exercises: [{ exerciseId: 'ex_bench', sets: [
        { kg: 80, reps: 14, done: true, setType: 'main' },
        { kg: 90, reps: 14, done: true, setType: 'main' },
        { kg: 95, reps: 12, done: true, setType: 'main' },
      ] }],
    },
  };

  const snap = buildMaxCycleSnapshot({ cycle, cache, exList, todayKey: '2026-05-11' });
  const bench = snap.benchmarks[0];

  assert.equal(bench.latest, null);
  assert.equal(bench.onPlan, null);
  assert.equal(snap.completed, 0);
  assert.equal(bench.actuals.length, 0);
  assert.equal(bench.baselineLatest.kg, 95);
  assert.equal(bench.baselineLatest.reps, 12);
});

test('overlayCurrentWorkoutDay · 오늘 cache가 있어도 현재 편집 중 세트를 SSOT로 덮는다', () => {
  const todayKey = '2026-05-11';
  const cache = {
    [todayKey]: {
      memo: 'saved day',
      exercises: [{ exerciseId: 'ex_bench', sets: [{ kg: 80, reps: 12, done: true }] }],
    },
    '2026-05-04': {
      exercises: [{ exerciseId: 'ex_bench', sets: [{ kg: 75, reps: 12, done: true }] }],
    },
  };
  const currentExercises = [
    { exerciseId: 'ex_bench', movementId: 'barbell_bench', sets: [{ kg: 100, reps: 12, done: true }] },
  ];
  const merged = overlayCurrentWorkoutDay(cache, todayKey, currentExercises);
  const cycle = {
    id: 'cycle_current_overlay',
    status: 'active',
    todayTrack: 'M',
    startDate: '2026-05-04',
    weeks: 6,
  ...
  const snap = buildRenderedMaxCycleSnapshot({
    cycle,
    cache: merged,
    exList: [{ id: 'ex_bench', movementId: 'barbell_bench', name: '바벨 벤치프레스' }],
    todayKey,
  });

  assert.equal(merged[todayKey].memo, 'saved day');
  assert.equal(merged[todayKey].exercises, currentExercises);
  assert.equal(cache[todayKey].exercises[0].sets[0].kg, 80);
  assert.equal(snap.benchmarks[0].latest.kg, 100);
  assert.equal(snap.benchmarks[0].onPlan, true);
});
...
test('resolveMaxBenchmarkPickerItems · 같은 공통 movementId 후보는 현재 헬스장 대표 하나로 접는다', () => {
  const cycle = {
    id: 'cycle_picker_dedupe',
    status: 'active',
    framework: 'dual_track_progression_v2',
    startDate: '2026-05-04',
    weeks: 6,
  ...
  const exList = [
    ...
    { id: 'ex_global_db', name: '덤벨 벤치프레스', movementId: 'dumbbell_bench', muscleId: 'chest', gymTags: ['*'] },
    { id: 'ex_moon_db', name: '덤벨 벤치프레스', movementId: 'dumbbell_bench', muscleId: 'chest', gymTags: ['gym_moon'] },
    { id: 'ex_dips', name: '딥스', movementId: 'dips', muscleId: 'chest', gymTags: ['*'] },
  ];
  ...
  const dumbbellItems = items.filter(item => item.exercise.movementId === 'dumbbell_bench');

  assert.equal(dumbbellItems.length, 1);
  assert.equal(dumbbellItems[0].exercise.id, 'ex_moon_db');
...
test('resolveMaxBenchmarkPickerItems · 테스트모드 운동추가는 같은 movementId의 등록 종목도 exerciseId별로 유지한다', () => {
  const cycle = {
    id: 'cycle_picker_same_movement_registered',
    status: 'active',
    framework: 'dual_track_progression_v2',
    startDate: '2026-05-04',
    weeks: 6,
  ...
  const exList = [
    { id: 'ex_bench', name: '바벨 벤치프레스', movementId: 'barbell_bench', muscleId: 'chest', gymTags: ['gym_moon'] },
    { id: 'bicep_1', name: '바벨 컬', movementId: 'barbell_curl', muscleId: 'bicep', gymTags: ['*'] },
    { id: 'custom_armcurl', name: '암컬', movementId: 'barbell_curl', muscleId: 'bicep', gymId: 'gym_moon', gymTags: ['gym_moon'] },
  ];
  ...
  assert.equal(scopedItems.filter(item => item.exercise.movementId === 'barbell_curl').length, 0);
  ...
  const curlIds = allRegisteredItems
    .filter(item => item.exercise.movementId === 'barbell_curl')
    .map(item => item.exercise.id)
    .sort();

  assert.equal(allRegisteredItems[0].kind, 'benchmark');
  assert.deepEqual(curlIds, ['bicep_1', 'custom_armcurl']);
  assert.ok(curlIds.every(id => allRegisteredItems.find(item => item.exercise.id === id)?.kind === 'exercise'));
});

test('resolveMaxBenchmarkPickerItems · 테스트모드 운동추가는 다른 헬스장 등록 종목도 일반모드 카탈로그처럼 유지한다', () => {
  const cycle = {
    id: 'cycle_picker_cross_gym_registered',
    status: 'active',
    framework: 'dual_track_progression_v2',
    startDate: '2026-05-04',
    weeks: 6,
  ...
  const movements = [
    ...MOVEMENTS_FIXTURE,
    { id: 'barbell_curl', nameKo: '바벨 컬', primary: 'bicep', subPattern: 'bicep', equipment_category: 'barbell' },
    { id: 'cable_curl', nameKo: '케이블 컬', primary: 'bicep', subPattern: 'bicep', equipment_category: 'cable' },
  ];
  const exList = [
    { id: 'ex_bench', name: '바벨 벤치프레스', movementId: 'barbell_bench', muscleId: 'chest', gymTags: ['gym_gangnam'] },
    { id: 'bicep_1', name: '바벨 컬', movementId: 'barbell_curl', muscleId: 'bicep', gymTags: ['*'] },
    { id: 'custom_armcurl', name: '암컬', movementId: 'barbell_curl', muscleId: 'bicep', gymId: 'gym_moon', gymTags: ['gym_moon'] },
    { id: 'custom_cable', name: '케이블', movementId: 'cable_curl', muscleId: 'bicep', gymId: 'gym_moon', gymTags: ['gym_moon'] },
  ];
...
test('buildMaxBenchmarkPickerEntry · 벤치마크 카드는 첫 입력 행만 만들고 전체 처방은 메타에 보존한다', () => {
  const cycle = {
  ...
  assert.equal(entry.maxPrescription.targetSets, 3);
  assert.equal(entry.maxPrescription.sets.length, 3);
  assert.ok(entry.maxPrescription.sets.every(set => set.kg === 92.5 && set.reps === 6));
  assert.equal(entry.sets.length, 1);
  assert.deepEqual(entry.sets[0], { kg: 92.5, reps: 6, setType: 'main', done: false, rpe: 9 });
});
...
test('buildMaxBenchmarkPickerEntry · 볼륨 단일 벤치마크는 H 대안을 만들지 않는다', () => {
  ...
  assert.equal(entry.maxPrescription.sets.length, 4);
  assert.equal(entry.sets.length, 1);
...
test('renderMaxPlanEditor · 활성 공통/공통 출처가 달라도 같은 공통 movementId는 하나로 접는다', () => {
  const html = renderMaxPlanEditor({
    cycle: {
      weeks: 6,
      benchmarks: [{
        id: 'bm_chest_db',
        exerciseId: 'ex_common_db',
        movementId: 'dumbbell_bench',
        label: '덤벨 벤치프레스',
        primaryMajor: 'chest',
        startKg: 30,
    ...
    movements: [
      {
        ...
        primary: 'chest',
        equipment_category: '',
        movementEquipmentCategory: 'dumbbell',
        gymTags: [],
        benchmarkDefaults: { source: 'exact', sessions: 1 },
        optionLabel: '가슴 · 덤벨 벤치프레스 · 공통 · 덤벨',
      },
      {
        id: 'movement:dumbbell_bench',
        exerciseId: null,
        movementId: 'dumbbell_bench',
        nameKo: '덤벨 벤치프레스',
        primary: 'chest',
        equipment_category: '덤벨',
        gymTags: ['*'],
        benchmarkDefaults: { source: 'legacy', sessions: 4 },
        optionLabel: '가슴 · 덤벨 벤치프레스 · 활성 공통 · 덤벨',
      },
    ],
  });
  const labels = [...html.matchAll(/<option[^>]*>(.*?)<\/option>/g)]
    .map(match => match[1].replace(/<[^>]*>/g, ''));
  const dumbbellLabels = labels.filter(label => label.includes('덤벨 벤치프레스'));

  assert.equal(dumbbellLabels.length, 1);
  assert.match(html, /value="ex_common_db" selected/);
  assert.doesNotMatch(html, /value="movement:dumbbell_bench"/);
});
...
test('renderMaxCycleDashboard · 성장 계단 카드 + 벤치마크 카드를 항상 렌더하고 미리보기/추천은 렌더하지 않는다', () => {
  const cycle = {
    version: 1,
    startDate: '2026-05-04',
    weeks: 6,
  ...
  const html = renderMaxCycleDashboard({
    ...
    cache: {},
    history: [{
      cycleId: 'c0', startDate: '2026-03-23', endDate: '2026-05-03', weeks: 6, settledAt: 1, grown: 1, held: 0,
      benchmarks: [{
        id: 'bm_chest_barbell_bench', movementId: 'barbell_bench', label: '바벨 벤치프레스', primaryMajor: 'chest',
        program: 'linear', decision: 'grow',
        representative: { kind: 'startKg', before: 72.5, after: 75, incrementKg: 2.5 },
        latest: null,
      }],
    }],
  });
  // 성장 예약 카드: 벤치마크별 현재 → 정산 후 무게 + 지난 정산 누적
  assert.match(html, /wt-v4-growth-summary/);
  assert.match(html, /성장 예약/);
  assert.match(html, /75 → 77\.5kg/);
  assert.match(html, /지난 정산 1회 · 누적 \+2\.5kg/);
  assert.match(html, /정산 D-/);
  assert.match(html, /정산 시 성장/);
  // 프로그램 진입점이 행 안에 노출된다
  assert.match(html, /계획·프로그램 설정/);
  // 벤치마크 카드는 항상 렌더
  assert.match(html, /wt-v4-benchmark-card/);
  assert.match(html, /오늘 열릴 벤치마크/);
  // 제거된 표면: 성장판 미리보기/추천/조언/곡선 궤적
  assert.doesNotMatch(html, /wt-v4-growth-preview/);
  assert.doesNotMatch(html, /wt-v4-rec-panel/);
  assert.doesNotMatch(html, /wt-v4-entry-chart/);
  assert.doesNotMatch(html, /마지막 10분 보강/);
  assert.doesNotMatch(html, /wt-v4-last-ten/);
  assert.doesNotMatch(html, /다음 동일 부위 Day 제안/);
});

test('renderMaxCycleDashboard · W6 도달 시 정산 CTA를 노출한다', () => {
  const cycle = {
    version: 1,
    startDate: '2026-05-04',
    weeks: 6,
  ...
  const html = renderMaxCycleDashboard({
    cycle,
    exList: [{ id: 'ex_bench', movementId: 'barbell_bench' }],
    todayKey: '2026-06-10', // W6
    cache: {},
  });
  assert.match(html, /data-action="settle-max-cycle"/);
  assert.match(html, /6주 정산하기/);
});

test('renderMaxCycleDashboard · 오늘 트랙과 마지막 10분 보강 카드를 렌더하지 않는다', () => {
  const cycle = {
    version: 1,
    startDate: '2026-05-04',
    weeks: 6,
    benchmarks: [{
      ...
      incrementKg: 2.5,
      tracks: {
        M: { enabled: true, startKg: 75, targetKg: 77.5, targetReps: 12, incrementKg: 2.5 },
        H: { enabled: true, startKg: 80, targetKg: 85, targetReps: 6, incrementKg: 2.5 },
      },
    }],
  };
  const html = renderMaxCycleDashboard({
  ...
  assert.match(html, /wt-v4-board/);
  assert.match(html, /wt-v4-benchmark-card/);
  assert.doesNotMatch(html, /wt-v4-track-card/);
  assert.doesNotMatch(html, /data-action="set-max-track"/);
  assert.doesNotMatch(html, /벤치마크별로도 볼륨\/강도 전환/);
  assert.doesNotMatch(html, /마지막 10분 보강/);
  assert.doesNotMatch(html, /wt-v4-last-ten/);
});
...
test('renderMaxGrowthPreview · 직전/직직전은 선택 조합 날짜가 아니라 해당 벤치마크 수행일을 쓴다', () => {
  ...
  assert.match(html, /기준 강도 5\/11 · 바벨 벤치프레스 · 3set x 82\.5kg x 6reps/);
  assert.match(html, /이전 기준 볼륨 5\/1 · 바벨 벤치프레스 · 4set x 75kg x 10reps/);
  assert.doesNotMatch(html, /직전 강도/);
  assert.doesNotMatch(html, /직직전 기록 없음/);
  assert.doesNotMatch(html, /기준 대비 82 · 기록 없음/);
});

test('renderMaxGrowthPreview · 기준 기록과 주차 계획 미수행을 같은 카드에서 구분한다', () => {
  const movements = [
    { id:'barbell_bench', nameKo:'바벨 벤치프레스', primary:'chest', subPattern:'chest_mid', equipment_category:'barbell' },
  ];
  const exList = [
  ...
  const comparison = {
    majors: ['chest'],
    today: { dateKey:'2026-05-11' },
    previous: [{ dateKey:'2026-04-30' }],
    deltas: [],
    imbalance: null,
  };
  const snapshot = {
    ...
    benchmarks: [{
      ...
      primaryMajor: 'chest',
      activeTrack: 'M',
      baselineLatest: { dateKey:'2026-04-30', kg:95, reps:12, track:'M' },
      tracks: {
        M: { startKg: 100, targetKg: 102.5, incrementKg: 2.5, startReps: 12, targetReps: 12, enabled: true },
        H: { startKg: 110, targetKg: 112.5, incrementKg: 2.5, startReps: 8, targetReps: 6, enabled: true },
      },
      planned: { plannedKg: 100, targetReps: 12 },
      plannedByTrack: {
        M: { plannedKg: 100, targetReps: 12 },
        H: { plannedKg: 110, targetReps: 6 },
      },
    }],
  };

  const html = renderMaxGrowthPreview({ comparison, cache, exList, majors: ['chest'], movements, snapshot });

  assert.match(html, /기준 볼륨 4\/30 · 바벨 벤치프레스/);
  assert.match(html, />W0</);
  assert.match(html, /95kg/);
  assert.match(html, /4\/30[\s\S]*기준/);
  assert.ok(html.indexOf('>W0<') < html.indexOf('>W1<'));
  assert.doesNotMatch(html, /data-action="select-max-plan-step"[^>]*data-week="0"/);
  assert.match(html, /계획[\s\S]*미수행/);
  assert.doesNotMatch(html, /직전 볼륨 4\/30/);
});
...
test('renderMaxGrowthPreview · 벤치마크가 없을 때도 부위별 최신 수행일 2개를 따로 찾는다', () => {
  ...
  assert.match(html, /직전 볼륨 5\/11 · 바벨 컬 · 3set x 30kg x 15reps/);
  assert.match(html, /직직전 볼륨 5\/4 · 바벨 컬 · 3set x 27\.5kg x 15reps/);
