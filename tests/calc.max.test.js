...
test('renderMaxCycleDashboard · Day 제안은 위, 오늘 보강 종목은 벤치마크 아래에 표시된다', () => {
  const cycle = {
    version: 1,
    startDate: '2026-05-04',
    weeks: 6,
  ...
  const html = renderMaxCycleDashboard({
    ...
    todayKey: '2026-05-11',
    cache: {},
    nextAdviceHtml: '<section class="wt-v4-next-day-coach">다음 동일 부위 Day 제안</section>',
    recommendationHtml: '<section class="wt-v4-rec-panel">오늘 보강 종목</section>',
  });
  assert.ok(html.indexOf('wt-v4-next-day-coach') < html.indexOf('wt-v4-lift-list'));
  assert.ok(html.indexOf('wt-v4-lift-list') < html.indexOf('wt-v4-rec-panel'));
});
