...
function buildHarnessScript() {
  ...
  return `
    ...
    // 완료 토글 스텁: 기존 테스트는 호출 시점의 세트 값만 검증한다. 번들의 실제
    // 구현은 __realToggleWorkoutSetDone으로 보관해 전체 경로 테스트에서 되살린다.
    window.__realToggleWorkoutSetDone = _toggleWorkoutExerciseSetDoneFromSheet;
    _toggleWorkoutExerciseSetDoneFromSheet = function (targetKey, targetSessionIndex, exerciseIndex, setIndex) {
